...
export default function InvestorDashboard({ user, onLogout }) {
  ...
  const handlePlaceBid = async (projectId) => {
    ...
    } catch (error) { if (error.response?.data?.needsPurchase) setShowPurchaseModal(true); else showPopup('Failed to place bid: ' + (error.response?.data?.message || 'Unknown error'), 'error'); }
  ...
  const openViewDetails = (project) => { setViewingProject(project); trackProjectView(project.id); };
    ...
    <div className="dashboard">
      <div className="dashboard-sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          <div className={`nav-item ${activeTab === 'forum' ? 'active' : ''}`} onClick={() => setActiveTab('forum')}><span className="nav-icon">🗣️</span><span className="nav-text">Community Forum</span></div>
      ...
      <div className="dashboard-main">
        <header className="dashboard-header">
          <div className="header-left"><h1 className="page-title">{activeTab === 'dashboard' && 'Investor Dashboard'}{activeTab === 'browse' && 'Browse Projects'}{activeTab === 'bids' && 'My Bids'}{activeTab === 'funded' && 'Funded Projects'}{activeTab === 'payments' && 'Payment History'}{activeTab === 'forum' && 'Community Forum'}{activeTab === 'profile' && 'Profile Settings'}</h1><p className="welcome-message">Welcome, {user.name} 👋</p></div>
        ...
        <div className="dashboard-content">
          ...
          {activeTab === 'browse' && (
            <div className="browse-projects">
              ...
              <div className="projects-grid">{projects.map(project => (<div key={project.id} className="project-card" ref={el => projectCardRefs.current[project.id] = el} data-project-id={project.id}><div className="project-header"><h3>{project.title}</h3><span className="project-category">{project.category}</span></div><p className="project-description">{project.description?.substring(0,150)}...</p><div className="project-meta"><div className="project-student"><strong>Student:</strong> {project.student_name}</div>{project.funding_goals && <div className="project-funding"><strong>Funding Goal:</strong> ₨ {project.funding_goals}</div>}</div><div className="project-technologies"><strong>Technologies:</strong> {project.technologies || 'Not specified'}</div><div className="project-stats"><span className="stat">👁️ {project.views || 0} views</span><span className="stat">❤️ {project.likes || 0} likes</span><button className={`like-btn ${project.user_has_liked ? 'liked' : ''}`} onClick={() => handleLike(project.id)}>{project.user_has_liked ? '❤️ Liked' : '🤍 Like'}</button></div><div className="project-actions"><button className="btn btn-primary" onClick={() => setSelectedProject(project)} disabled={bidCredits.totalBids === 0}>Place Bid {bidCredits.totalBids === 0 && '(No credits)'}</button><button className="btn btn-outline" onClick={() => openViewDetails(project)}>View Details</button></div></div>))}</div>
            </div>
          )}
          ...
          {activeTab === 'forum' && <div className="community-forum"><h2>Community Forum</h2><p>Community forum feature coming soon!</p></div>}
          {activeTab === 'profile' && (
    ...
    <div className="modal-content">
      ...
      <div className="modal-body">
        ...
        <div className="bid-packs">
          {/* Always use hardcoded packs for reliability */}
  ...
  <div className="modal-overlay">
    <div className="modal-content" style={{ maxWidth: '450px' }}>
      ...
      <style>{`
        ...
        .stat-card { background: white; padding: 20px; border-radius: 12px; display: flex; align-items: center; gap: 15px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); }
        .stat-icon { font-size: 2rem; }
        .stat-number { font-size: 1.5rem; font-weight: bold; color: #2d6374; }
        ...
        .project-header { padding: 20px 20px 12px 20px; background: linear-gradient(135deg, #f8fafc 0%, #ffffff 100%); border-bottom: 1px solid #eef2f5; }
